...
def main():
    ...
    print("[{time}][INFO]:\tListed all files from filterd_folder".format(time=datetime.now().strftime("%H:%M:%S")))
    
    #for root, dirs, filenames in os.walk(sorted_folder):
    #    sorted_files.extend(filenames)
    #print("[{time}][INFO]:\tListed all files from sorted_folder".format(time=datetime.now().strftime("%H:%M:%S")))

    pool = multiprocessing.Pool(processes=num_workers)

    print("[{time}][INFO]:\tStart extraction using filtered files".format(time=datetime.now().strftime("%H:%M:%S")))
    pool.map(process_filtered, filtered_files)
    print("\33[92m[{time}][INFO]:\33[1m\tCompleted extraction from filtered files".format(time=datetime.now().strftime("%H:%M:%S")))
       
    #print("[{time}][INFO]:\tStart extraction using sorted files".format(time=datetime.now().strftime("%H:%M:%S")))
    #pool.map(process_sorted, sorted_files)
    #print("\33[92m[{time}][INFO]:\33[1m\tCompleted extraction from sorted files".format(time=datetime.now().strftime("%H:%M:%S")))
    
    pool.close()
    pool.join()
    print("\33[96m[{time}][INFO]:\33[94m\tExtraction completed".format(time=datetime.now().strftime("%H:%M:%S")))
