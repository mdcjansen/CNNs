...
models_path = r'D:/path/to/model.pt'
input_modeldata = r"D:/path/to/model.csv"
study_summary_path = r"D/path/to/output/study_summary.csv"
...
def calculate_metrics(y_true, y_pred, y_proba, all_id):
        ...
        for f in files:
            if f.endswith(".csv"):
                ...
                with open(csv_path, 'r') as param_file:
                    details = csv.DictReader(param_file)
                    for d in details:
                        input_param.setdefault(d['model_name'], []).append(d['batch_norm'])
                    param_file.close()
...
def test(model, test_data_loader, device):
    ...
    with torch.no_grad():
        ...
        with open(study_summary_path, 'w+', newline='') as csvfile:
...
if __name__ == '__main__':
    ...
    np.random.seed(seed)
    random.seed(seed)
    summary_csv = pd.DataFrame(columns=['Target', 'Model', 'wandb name', 'Acc', 'F1', 'AUC',
                                        'Bal Acc', 'Precision', 'Recall', 'CM-1', 'CM-2'])
    ...
    for mdl_path, mdl_name in zip(model_paths, model_names):
        ...
        print("Main complete")
        summary_csv.to_csv(os.path.join(models_path, 'testset_performance.csv'))
